...
axios.get('https://raw.githubusercontent.com/KanzuWakazaki/LazicTeam/main/package.json').then((res) => {
...
if (global.config.DeveloperMode == false && client.timeLoadModule.length != 0) logger.loader(client.timeLoadModule, "warn");
writeFileSync(client.dirConfig, JSON.stringify(configValue, null, 4), 'utf8');
unlinkSync(client.dirConfig + ".temp");

logger.loader(`[ []==> Lazic Team <==[]] ]`);
...
(async () => {
	try {
		...
		logger("Kết nối cơ sở dữ liệu thành công", "[ Lazic ]");
		const models = require("./includes/database/model");
		return onBot({ models });
	}
	catch (error) {
		() => logger(`Kết nối cơ sở dữ liệu thất bại, Lỗi: ${error.name}: ${error.message}`, "[ Lazic ]");
	}
})();
