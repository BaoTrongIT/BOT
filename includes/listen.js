module.exports = function({ api, client, global, models, timeStart }) {
	...
	(async function() {
		try {
			...
			logger("Khởi tạo biến môi trường thành công!", "[ Lazic ]");
		}
		catch (error) {
	...
	logger(`${api.getCurrentUserID()} - [ ${global.config.PREFIX} ] • ${(!global.config.BOTNAME) ? "This bot convert by : LazicTeam" : global.config.BOTNAME}`, "[ UID ]");
