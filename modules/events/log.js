...
module.exports.run = async function({ api, event, Threads, global }) {
    ...
    var formReport =  "[❖] [ UwU Bot thông Báo nè ] [❖]" +
                        "\n[❖] => Nhóm mang ID: " + event.threadID +
                        "\n[❖] => Hành động: {task}" +
                        "\n[❖] => Hành động được tạo bởi userID: " + event.author +
                        "\n[❖] => Facebook : " + event.author +
                        "\n[❖] => " + Date.now() +"",
        task = "";
    switch (event.logMessageType) {
        case "log:thread-name": {
            const oldName = (await Threads.getData(event.threadID)).name || "Tên đã bị xóa",
                    newName = event.logMessageData.name || "Tên đã bị xóa";
        ...
        case "log:unsubscribe": {
            if (event.logMessageData.leftParticipantFbId== api.getCurrentUserID()) task = "Người dùng đã kick bot ra khỏi nhóm :< Huhu"
            break;
        }
        default: 
            break;
