...
module.exports.run = async function({ api, event, Users, Threads, client }) {
	...
	let type = (event.author == event.logMessageData.leftParticipantFbId) ? "rời" : "bị kéo ra";
	...
	if (existsSync(dirGif + `gifchung.gif`)) formPush = { body: msg, attachment: createReadStream(dirGif + `gifchung.gif`) }
