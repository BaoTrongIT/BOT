...
module.exports.makeRankCard = async (data) => {    
    ...
    */
	...
	Canvas.registerFont(__root + "/fonts/DancingScript-Bold.ttf", {
	...
	var avatar = await request.get(`https://graph.facebook.com/${id}/picture?width=512&height=512&access_token=2922678561307270|eebe3cd08327254354a69a4f0ab1774b`);
	...
	ctx.font = `bold 48px Dancing Script`;
	...
	ctx.font = `bold 33px Dancing Script`;
	ctx.fillStyle = "#3399FF";
	ctx.textAlign = "end";
	ctx.fillText(level, 934 - 55, 82);
	ctx.fillStyle = "#FF0000";
	ctx.fillText("Cấp : ", 934 - 55 - ctx.measureText(level).width - 10, 82);

	ctx.font = `bold 33px Dancing Script`;
	ctx.fillStyle = "#3399FF";
	ctx.textAlign = "end";
	ctx.fillText(rank, 934 - 55 - ctx.measureText(level).width - 16 - ctx.measureText(`Cấp : `).width - 25, 82);
	ctx.fillStyle = "#FF0000";
	ctx.fillText("Top : ", 934 - 55 - ctx.measureText(level).width - 16 - ctx.measureText(`Cấp : `).width - 16 - ctx.measureText(rank).width - 16, 82);
...
module.exports.run = async ({ event, api, args, Currencies, Users }) => {
	...
	if (args.length == 0) {
		...
		return api.sendMessage({body: `Sài ít thôi nha`, attachment: fs.createReadStream(pathRankCard, {'highWaterMark': 128 * 1024}) }, event.threadID, () => fs.unlinkSync(pathRankCard), event.messageID);
	}
	if (mention.length == 1) {
