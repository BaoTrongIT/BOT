	...
	if (body.indexOf("Sẵn sàng") == 0) {
		...
		if (values.player.length == values.ready) {
			...
			return api.sendMessage(
				"[ = ] [ Kết quả ] [ = ]" +
				"\n" + ranking +
				"\n Người Thua Phải Trả Lời Câu Hỏi Do Người Thắng Hỏi !"
			,threadID);
		}
		else return
	}
...
module.exports.run = async ({ api, event, args, client, utils }) => {
	...
	else return api.sendMessage("bấm /howtouse baicao", event.threadID, event.messageID);
}