...
module.exports.run = async function({ api, event, args, global, client }) {
	...
	if (urlValid) {
		...
		try {
			...
			if (error.statusCode == "404") return api.sendMessage("Không tìm thấy bài nhạc của bạn thông qua link trên ", event.threadID, event.messageID);
		...
		try {
			...
			return api.sendMessage(`[✤] => Có ${link.length} Kết Quả Trùng Với Từ Khoá Tìm Kiếm Của Bạn <= [✤]\n❖━━━━━━━━━━━━━━━━━━━━━━❖\n${msg}[✤] => Hãy Trả Lời Tin Nhắn Bot Theo Số Thứ Tự`, event.threadID,(error, info) => client.handleReply.push({ name: this.config.name, messageID: info.messageID, author: event.senderID, link }), event.messageID);
		}
		catch (error) {
			api.sendMessage("Không thể xử lý request do dã phát sinh lỗi: " + error.message, event.threadID, event.messageID);
		}
