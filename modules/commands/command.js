...
const load = async ({ name, event, api, client, global, loadAll }) => {
	...
	try {
		...
		return api.sendMessage(`[ ❖ ] => Không có : ${name}.js`, event.threadID, event.messageID);
	}
	...
	try {
		...
		if (client.commands.has(command.config.name)) throw new Error(`[ ❖ ] => Bị Trùng Tên Nè`);
		...
		if (command.config.envConfig) {
			try {
				...
				logger.loader(`[ ❖ ] => Đã Tải Thành Công : ${command.config.name}`)
			} catch (error) {
				console.log(error);
				logger.loader(`[ ❖ ] => Bị lỗi gòi k load dc ${command.config.name}`, "error");
			}
		}
		...
		else return api.sendMessage(`Đã Tải Thành Công : ${command.config.name}!`, event.threadID);
	}
	catch (error) {
		logger.loader(`Tải lỗi ${name} với lỗi: ${error.name}:${error.message}`, "error");
		if (loadAll == true) return
		else return api.sendMessage(`Tải lỗi ${name} với lỗi: ${error.name}:${error.message}`, event.threadID);
	}
}
...
module.exports.run = async ({ event, api, global, client, args, utils }) => {
	...
	switch (args[0]) {
		...
		break;
		default:
			api.sendMessage("Uhmmm M cố tình sai chứ j :)?", event.threadID, event.messageID)
		break;
	}
}