...
module.exports.event = async function({ api, event, Currencies, Users, client }) {
	...
	if (level > curLevel && level != 1) {
		...
		var messsage = (typeof threadData.customRankup == "undefined") ? msg = "U là trời , [ {name} ] Đã Đạt Đến level [ {level} ] , Chúc bạn 1 ngày tốt lành ! " : msg = threadData.customRankup,
			arrayContent;
		...
		if (existsSync(__dirname + `/cache/rankup/gifchung.gif`)) arrayContent = { body: messsage, attachment: createReadStream(__dirname + `/cache/rankup/gifchung.gif`), mentions: [{ tag: nameUser, id: senderID }] };
